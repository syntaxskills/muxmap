...
test('Claude idle prompts downgrade stuck states but preserve needs input', () => {
  ...
  try {
    ...
    manager.acknowledge(session.id, '2026-08-07T11:05:00.000Z')
    assert.equal(manager.decorate([session])[0].agent?.state, 'delegated')
    manager.acknowledge(session.id, '2026-08-07T13:05:01.000Z')
    assert.equal(manager.decorate([session])[0].agent?.state, 'read')
    manager.recordAgentEvent({ backend: 'tmux', paneId: '%21' }, 'claude', { hook_event_name: 'PreToolUse' }, '2026-08-07T13:06:00.000Z')
    assert.equal(manager.decorate([session])[0].agent?.state, 'working')
    manager.recordAgentEvent({ backend: 'tmux', paneId: '%21' }, 'claude', { hook_event_name: 'Stop', background_tasks: [{ id: 'task-1', description: 'Run mvn test' }] }, '2026-08-07T13:07:00.000Z')
    assert.equal(manager.decorate([session])[0].agent?.state, 'delegated')
    manager.recordAgentEvent({ backend: 'tmux', paneId: '%21' }, 'claude', { hook_event_name: 'Notification', notification_type: 'idle_prompt' }, '2026-08-07T13:07:15.000Z')
    assert.equal(manager.decorate([session])[0].agent?.state, 'delegated')
    manager.recordAgentEvent({ backend: 'tmux', paneId: '%21' }, 'claude', { hook_event_name: 'Stop', background_tasks: [{ id: 'task-1', description: 'Run mvn test' }] }, '2026-08-07T13:07:30.000Z')
    assert.equal(manager.decorate([session])[0].agent?.state, 'delegated')
    manager.recordAgentEvent({ backend: 'tmux', paneId: '%21' }, 'claude', { hook_event_name: 'TaskCompleted', task_id: 'task-1' }, '2026-08-07T13:08:00.000Z')
    assert.equal(manager.decorate([session])[0].agent?.state, 'delegated')
    manager.recordAgentEvent({ backend: 'tmux', paneId: '%21' }, 'claude', { hook_event_name: 'Stop', background_tasks: [{ id: 'artifact-1', type: 'monitor', description: 'live updates for artifact demo' }] }, '2026-08-07T13:08:30.000Z')
    ...
    manager.recordAgentEvent({ backend: 'tmux', paneId: '%21' }, 'claude', { hook_event_name: 'Notification', notification_type: 'idle_prompt' }, '2026-08-07T13:08:45.000Z')
    assert.equal(manager.decorate([session])[0].agent?.state, 'standby')
    manager.acknowledge(session.id, '2026-08-07T13:09:00.000Z')
    assert.equal(manager.decorate([session])[0].agent?.state, 'read')
    manager.recordAgentEvent({ backend: 'tmux', paneId: '%21' }, 'claude', { hook_event_name: 'PreToolUse' }, '2026-08-07T13:09:30.000Z')
    assert.equal(manager.decorate([session])[0].agent?.state, 'working')
    manager.recordAgentEvent({ backend: 'tmux', paneId: '%21' }, 'claude', { hook_event_name: 'Stop', background_tasks: [], session_crons: [] }, '2026-08-07T13:10:00.000Z')
    assert.equal(manager.decorate([session])[0].agent?.state, 'completed')
  } finally {
    store.close()
    rmSync(directory, { recursive: true, force: true })
