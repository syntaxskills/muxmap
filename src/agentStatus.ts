...
const states: Record<AgentActivity['state'], string> = { unavailable: 'Status unavailable', working: 'Working', needs_input: 'Needs input', completed: 'Completed', read: 'Read' }
...
export function agentStatusText(agent: AgentActivity, now = Date.now()) {
  ...
  const duration = !agent.since || !['working', 'needs_input'].includes(agent.state) ? '' : minutes < 1 ? ' <1m' : minutes < 60 ? ` ${minutes}m` : ` ${Math.floor(minutes / 60)}h ${minutes % 60}m`
