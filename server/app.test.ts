...
test('file preview API renders markdown and html with MuxMap actions', async () => {
  ...
  try {
    ...
    const sourceHref = htmlPreview.match(/href="([^"]+)">Source<\/a>/)?.[1].replace(/&amp;/g, '&')
    assert.ok(sourceHref)
    const htmlSource = await fetch(new URL(sourceHref, `${base}/api/files/open`), { headers: { cookie } })
    assert.equal(htmlSource.status, 200)
    assert.match(await htmlSource.text(), /&lt;h1&gt;Preview me&lt;\/h1&gt;/)
  } finally {
    await server.close()
    rmSync(root, { recursive: true, force: true })
  }
...
test('file preview API resolves terminal relative links against the live tmux pane cwd', async () => {
  ...
  try {
    ...
    assert.ok(html.includes(`<span>From node</span><a href="/?node=${node.id}"`))
    assert.match(html, />Live cwd links<\/a>/)
  } finally {
    await server.close()
    rmSync(root, { recursive: true, force: true })
  }
...
test('file previews link every referring node by resolved path and preserve navigation context', async () => {
  const root = realpathSync(mkdtempSync(join(tmpdir(), 'muxmap-file-nodes-')))
  const shared = join(root, 'shared.md')
  writeFileSync(shared, '# Shared document\n')
  mkdirSync(join(root, 'other'))
  writeFileSync(join(root, 'other', 'shared.md'), '# Different document\n')
  const server = createMuxMapServer({ databasePath: ':memory:', allowedRoots: [root], platform: 'linux', token: 'test-token', tmux: fakeTmux() })
  try {
    const address = await server.listen(0)
    const base = `http://127.0.0.1:${address.port}`
    const cookie = (await fetch(`${base}/api/auth`)).headers.get('set-cookie')?.split(';')[0] ?? ''
    const makeNode = (title: string) => server.store.createNode('default', { parentId: 'workspace', title, type: 'note' })
    const origin = makeNode('Origin <script> & "node"')
    const older = makeNode('Older reference')
    const absolute = makeNode('Absolute reference')
    const unrelated = makeNode('Same filename elsewhere')
    ...
    for (let index = 0; index < 201; index++) server.store.createNodeNote(older.id, { body: `Newer note ${index}` })
    addLink(absolute.id, `${base}${fileUrl(shared)}&line=1`)
    addLink(absolute.id, `${fileUrl(shared)}&line=2&renderer=source`)
    addLink(unrelated.id, fileUrl('shared.md', join(root, 'other')))
    addLink(stale.id, fileUrl('gone.md'))
    ...
    const preview = await fetch(previewUrl, { headers: { cookie } })
    assert.equal(preview.status, 200)
    const html = await preview.text()
    const nodeIds = [...html.matchAll(/href="\/\?node=([^"]+)"/g)].map((match) => match[1])
    assert.deepEqual(nodeIds, [origin.id, absolute.id, older.id])
    assert.match(html, /From node/)
    assert.match(html, /Also used by/)
    assert.match(html, /Origin &lt;script&gt; &amp; &quot;node&quot;/)
    const sourceHref = html.match(/href="([^"]+)">Source<\/a>/)?.[1].replace(/&amp;/g, '&')
    assert.ok(sourceHref)
    const sourceUrl = new URL(sourceHref, previewUrl)
    assert.equal(sourceUrl.searchParams.get('path'), shared)
    assert.equal(sourceUrl.searchParams.get('nodeId'), origin.id)
    assert.equal(sourceUrl.searchParams.get('sessionId'), 'gone-session')
    assert.equal(sourceUrl.searchParams.get('line'), '1')
    assert.equal(sourceUrl.searchParams.get('column'), '2')
    const source = await fetch(sourceUrl, { headers: { cookie } })
    assert.equal(source.status, 200)
    const sourceHtml = await source.text()
    assert.match(sourceHtml, /<tr id="L1" class="is-selected">/)
    assert.deepEqual([...sourceHtml.matchAll(/href="\/\?node=([^"]+)"/g)].map((match) => match[1]), nodeIds)

    const missing = await fetch(`${base}${fileUrl('gone.md')}&nodeId=${origin.id}`, { headers: { cookie, accept: 'text/html' } })
    assert.equal(missing.status, 404)
