...
import { nodeNoteDisplayText, nodeNoteLinkUrl, nodeNoteProviderLabel, visibleNodeNotes } from './nodeNotes.ts'
...
async function checkNoteLink(event: MouseEvent<HTMLAnchorElement>) {
  ...
  if (await checkFileNote(event.currentTarget.dataset.noteUrl) === 'File not found') details?.removeAttribute('open')
}
    ...
    <section className="node-notes-preview" aria-label="Node notes" onClick={stop} onPointerDown={stop}>
      ...
      {visible.map((note) => {
        ...
        return <CollapsibleFileNote key={note.id} note={note}>{note.url
          ? <a href={nodeNoteLinkUrl(note, window.location.origin)} data-note-url={note.url} target="_blank" rel="noopener noreferrer" onClick={checkNoteLink} onAuxClick={checkNoteLink}>{content}</a>
      ...
      <div className="node-note-list">
        ...
        {visibleNodeNotes(notes, 100).map((note) => editingId === note.id ? (
          ...
          <CollapsibleFileNote key={note.id} note={note}><article className={`node-note-item is-${note.provider}`}>
            ...
            <div>{note.url ? <a href={nodeNoteLinkUrl(note, window.location.origin)} data-note-url={note.url} target="_blank" rel="noopener noreferrer" onClick={checkNoteLink} onAuxClick={checkNoteLink}>{nodeNoteDisplayText(note)}<OpenInNewWindowIcon /></a> : <strong>{nodeNoteDisplayText(note)}</strong>}{note.body && note.label && <p>{note.body}</p>}<small>{note.createdBy.startsWith('terminal:') ? 'Opened from terminal' : note.createdBy === 'human' ? 'Edited by human' : 'Agent update'} · {new Date(note.updatedAt).toLocaleString()}</small></div>
