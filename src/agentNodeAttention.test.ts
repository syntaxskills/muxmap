...
test('agent node attention distinguishes working, completed, and input-needed states', () => {
  ...
  assert.match(css, /\.is-working\s*>\s*\.agent-icon\.is-pi\s*\{[^}]*animation:\s*agent-pi-working/s)
  assert.match(css, /@keyframes agent-pi-working\s*\{[\s\S]*drop-shadow/)
  assert.doesNotMatch(css.match(/@keyframes agent-pi-working\s*\{[\s\S]*?\n\}/)?.[0] ?? '', /rotate\(/)
